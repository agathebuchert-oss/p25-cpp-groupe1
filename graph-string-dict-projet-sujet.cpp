...
private :
    ...
    {      
        ...
        data.resize(n, std::vector<double>(n, inf));
...
public:
    ...
    void add_edge(const std::string &begin, const std::string &end, double value)
    {
        add_vertex(begin);
        add_vertex(end);
        int itx_begin = name_to_idx[begin];
        int itx_end = name_to_idx[end];
        v[itx_begin]->add_edge(new Edge(value, v[itx_begin], v[itx_end]));
    }
    void dfs(const std::string &name)
    {
        if (name_to_idx.find(name) != name_to_idx.end())
        {
            ...
            dfs_recursive(v[name_to_idx.find(name)->second], visited);
        }
        else
        {
            std::cout << "Sommet " << name << " inconnu." << std::endl;
        }
    }
...
private:
    ...
    void add_vertex(const std::string &name)
    {
        if (name_to_idx.find(name) == name_to_idx.end())
        {
            ...
            name_to_idx[name] = v.size() - 1;
        }
    }

    void dfs_recursive(Vertex *current, std::set<Vertex *> &visited)
    {
        visited.insert(current);
        std::cout << "Visite de : " << current->name << std::endl;
        for (Edge *e : current->edges)
        {
            Vertex *voisin = e->end;
            if (visited.find(voisin) == visited.end())
            {
                dfs_recursive(voisin, visited);
            }
        }
    }
    ...
    void compute_floyd_warshall() {
        ...
        for (int k = 0; k < n; ++k) {
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
                    if (M->data[i][k] + M->data[k][j] < M->data[i][j]) {
